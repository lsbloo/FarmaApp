...
public class DataLoader implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        Optional<UserAuth> userAuth = authenticatorService.findUserByEmail("osvaldo.airon@dcx.ufpb.br");
    ...
    private void createRolesIfNotFound() {
        ...
        for (TypeRoles typeRoles : rolesSystem) {
            Optional<Role> role = this.userService.findRoleByName(typeRoles);
            if (!role.isPresent()) {
                Role rolex = new Role();
                rolex.setName(typeRoles.toString());
                this.userService.saveRole(rolex);
            }
        }
    }
