...
cp_ant = _list_tables_antibiotics.copy()
...
def validate_med(medicamentos):
    for i in cp_ant[1:]:
        cell_str = str(i)
        s = cell_str.split(":")
        string_c = s[1].replace("'","")
        for k in medicamentos:
            if string_c == k[1].product_name or k[2].active_principle.find(string_c) != -1:
                medicamentos.remove(k)
...
validated_med = validate_med(medicamentos)

quantidade_dados_anvisa = len(cp)
quantidade_dados_medicamentos = len(validated_med)
quantidade_dados_antibioticos_filtrados = len(validated_med)
...
t2 = time.time()
d = sender_eligibles_endpoint(validated_med,antibioticos,token)
temp2 = time.time() - t2
minutos = temp2/60
horas = minutos/60
